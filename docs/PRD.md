...
> **버전**: 2.0 | **기준일**: 2026-03-17 | **범위**: MVP — Local-first (Offline-only)

---

> ### ⚡ v2.0 전략 전환 — Local-first + PWA (2026-03-17)
>
> **백엔드(.NET) 및 Supabase 제거. 모든 데이터는 브라우저 IndexedDB(Dexie.js)에 저장.**
>
> | 항목 | 변경 내용 |
> |------|---------|
> | DB | PostgreSQL → **IndexedDB (Dexie.js 3.x)** |
> | 백엔드 | .NET 8 Web API → **없음** |
> | 배포 | Render (프론트+백엔드) → **정적 사이트 (프론트만)** |
> | 오프라인 | 미지원 → **PWA 완전 지원 (vite-plugin-pwa 0.16.x)** |
> | 할부/반복 로직 | 서버 처리 → **프론트엔드 `src/database/db.ts` 헬퍼** |
> | 비즈니스 기획 | MonthStartDay, 할부, 반복, 카드 청구, 포인트 예산 **모두 유지** |
...
**기본 거래 입력**
- 금액, 날짜, 카테고리, 메모 입력
- 수입(Income) / 지출(Expense) / 저축(Savings) 구분
- 합산 포함 여부 선택 (포인트 사용 거래 등 집계 제외 가능)
- **결제/저축 수단 선택**: 수입은 수단 없음, 지출은 결제수단, 저축은 저축 수단 (§4.4 참조)

**거래 유형별 렌더링 규칙**

| 타입 | 금액 기호 | 색상 | 잔액 영향 |
|------|---------|------|---------|
| 수입 (Income) | + | 파란색 (text-blue-600) | +차감 |
| 지출 (Expense) | - | 빨간색 (text-red-500) | -차감 |
| 저축 (Savings) | - | 초록색 (text-emerald-500) | -차감 |

**거래 목록**
- 현재 월의 거래 내역을 날짜 역순으로 표시
- **검색/필터 아코디언**: 거래 목록 상단에 접히는 패널로 배치
  - 메모 키워드 검색 (대소문자 무시)
  - 유형 필터 칩 (수입 / 지출 / 저축)
  - 유형 선택 시 해당 유형의 카테고리 칩 추가 표시
  - 월 이동 시 필터 초기화
- **일별 서머리**: 날짜 헤더에 `+수입 / -지출 / -저축` 형식으로 색상 구분 표시
...
- 수입(Income) / 지출(Expense) / 저축(Savings) 카테고리 구분
- 사용자 직접 추가/삭제 가능
...
**저축 수단 (Savings 거래에 사용)**
- 저축 거래는 결제수단 대신 **저축 수단(은행 계좌)** 을 선택
- 기본 항목: 기업은행, 카카오뱅크, 현금
- 수입 거래에는 결제/저축 수단 없음

> **분리 원칙**: 저축은 돈이 빠져나가는 계좌(저축 수단)가 의미 있고, 일반 결제수단(카드/포인트)과 혼용되어서는 안 됨.
...
- 홈 화면 상단에 미등록 반복 항목을 배너로 표시
- 배너 항목 **탭** → 해당 반복 마스터 데이터로 거래 입력 폼 pre-fill (날짜=당월 예정일, 금액/카테고리/수단/메모 자동 입력). 저장 시 해당 반복 마스터와 연결되어 배너에서 자동 제거
- 배너 항목 **[×] 탭** → 삭제 옵션 바텀 시트 (단건 스킵 / 이후 중단 / 전체 삭제)

**반복 거래 관리 (홈 화면에서 직접 처리)**

- 설정 화면에 별도 관리 섹션 없음
- 반복 거래 삭제는 홈 화면 거래 목록에서 해당 항목 클릭 → 바텀 시트로 처리
- 원부 수정은 MVP 스코프 외 (삭제 후 재등록 방식)
...
| 항목 | 요구사항 |
|------|---------|
| 응답성 | 거래 목록 로드 1초 이내 (로컬 IndexedDB 쿼리 기준) |
| UI | 모바일 우선 (Mobile First), 최대 너비 512px |
| 반응형 | 모바일 / 태블릿 / PC 모두 대응 |
| 오프라인 | 인터넷 연결 없이 앱 실행 및 데이터 입력/조회 완전 동작 |
| PWA 설치 | 모바일 브라우저 "홈 화면에 추가"로 네이티브 앱처럼 설치 가능 |
| 데이터 무결성 | 포인트 차감/복구 원자성 보장 (Dexie transaction) |
| 데이터 무결성 | 반복 거래 중복 생성 방지: `getRecurringPending()` 생성 전 중복 체크 |
| 데이터 무결성 | 포인트 잔액 음수 방지: 저장 전 애플리케이션 레벨 검증 |
| 날짜 | 거래일은 `YYYY-MM-DD` 문자열로 저장 — 타임존 변환 없이 로컬 날짜 그대로 사용 |
| 보안 | MVP: 인증 없음 (단일 사용자·단일 기기 가정) |
| 배포 | Render 정적 사이트 자동 배포 (main 브랜치 push 시) |
...
- 사용자 인증 / 멀티 유저
- 기기 간 데이터 동기화 (클라우드 백업)
- 예산 한도 설정 및 초과 알림
- 금융사 자동 연동
- 영수증 이미지 첨부
- CSV/Excel 내보내기·가져오기 (데이터 백업용으로 향후 우선 검토)
...
#### Service Worker — 앱 껍데기(UI) 캐싱

`vite-plugin-pwa`가 빌드 시 `dist/sw.js`를 자동 생성한다.

| 시점 | 동작 |
|------|------|
| 최초 방문 | 서버에서 JS·CSS·HTML·이미지를 받아 Cache Storage에 저장 |
| 이후 방문 | 서비스 워커가 네트워크 요청을 가로채 캐시에서 즉시 반환 |
| 오프라인 | 캐시가 있으므로 인터넷 없이도 앱 UI가 즉시 로드됨 |
| 앱 업데이트 | `registerType: 'autoUpdate'` — 새 버전 감지 시 백그라운드에서 자동 갱신 |

#### IndexedDB (Dexie.js) — 데이터 영구 저장

모든 거래·설정·카테고리 데이터는 사용자 기기의 IndexedDB에 저장된다. 서버 통신 없이 즉시 읽기/쓰기가 가능하다.

| 테이블 | 역할 |
|--------|------|
| `transactions` | 단건·할부·반복 거래 내역 |
| `categories` | 수입/지출/저축 카테고리 (soft-delete) |
| `paymentMethods` | 결제수단 (카드 청구일·포인트 예산 포함) |
| `savingsMethods` | 저축 수단 |
| `recurringTransactions` | 반복 원부 |
| `recurringSkips` | 반복 단건 스킵 기록 |
| `installmentTransactions` | 할부 원부 |
| `userSettings` | 월 시작일 등 사용자 설정 |

스키마 정의 위치: `frontend/src/database/db.ts`

#### PWA 설치

모바일 브라우저로 접속 시 "홈 화면에 추가" 프롬프트가 표시된다. 설치 후에는 주소창 없이 `standalone` 모드로 실행되어 네이티브 앱과 동일한 UX를 제공한다.

### 7.3 오프라인 동작 범위

| 기능 | 오프라인 가능 여부 |
|------|-----------------|
| 앱 실행 및 UI 로드 | ✅ 서비스 워커 캐시 |
| 거래 입력·수정·삭제 | ✅ IndexedDB |
| 월별 요약·통계 조회 | ✅ IndexedDB 로컬 계산 |
| 카테고리·결제수단 관리 | ✅ IndexedDB |
| 반복 거래 자동 적용 | ✅ 프론트엔드 헬퍼 (`applyRecurringForMonth`) |
| 할부 분할 계산 | ✅ 프론트엔드 헬퍼 (`createInstallment`) |
| 기기 간 데이터 동기화 | ❌ MVP 스코프 외 |

### 7.4 데이터 보존 정책 및 주의사항

- **데이터 저장 위치**: 브라우저 IndexedDB (기기 로컬)
- **앱 삭제·재설치**: 데이터 유지됨 (브라우저 데이터와 별개)
- **브라우저 데이터 초기화 / "사이트 데이터 삭제"**: **데이터 영구 삭제** — 사용자에게 명확히 안내 필요
- **기기 교체·분실**: 데이터 복구 불가 — CSV 내보내기 기능으로 보완 예정 (스코프 외)
- **동일 기기 내 다른 브라우저**: 브라우저별로 IndexedDB가 분리되어 데이터 공유 불가
...
| 문서 | 내용 |
|------|------|
| `ROADMAP.md` | 스프린트 계획, DB 스키마 |
| `frontend/src/database/db.ts` | Dexie 스키마 + 할부/반복 헬퍼 |
