...
- ✅ `sprint7 → develop` 머지 완료 (2026-03-17, commit: 7353d16)
...
- Chart.js 도넛 + 막대 차트를 단일 Vue SFC 안에서 생명주기(watch/onUnmounted)로 안정적으로 관리했다.
- 홈 탭과 동일한 `txMonthFiltered`, `getMonthPeriod()` 유틸을 재사용하여 중복 계산 없이 통계 데이터를 구성했다.
- 수입 대비 %, 잔액 바, 범례 비율 표시 등 UI 개선을 목업 단계에서 선제적으로 반영하여 Step 2 승인을 한 번에 통과했다.
- `npm run build` TypeScript 오류 0건으로 빌드 완료.

### 아쉬운 점 / 주의사항

- MockupView.vue 파일 크기가 계속 증가 중(통계 섹션 추가로 더 커짐). Sprint 9 실서비스 이관 시 각 View로 분리 예정.
- 6개월 추이 계산이 `txTransactions` 전체 배열을 순회하므로 데이터 증가 시 성능 검토 필요(목업 단계이므로 현재는 무관).

### 다음 스프린트 연계

- Sprint 8: 설정 화면 목업 (카드 청구 설정, 저축 수단, 반복 거래, 카테고리, 결제수단 관리)
- Sprint 9: 전체 실서비스 구현 (Dexie.js 기반, 백엔드 없음)
