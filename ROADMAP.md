...
- **기술 스택**: CLAUDE.md 참조
- **PRD**: `docs/PRD.md` (v1.3, 2026-03-16 기준)
...
| # | 조건 | 도구 | 합격 기준 |
|---|------|------|----------|
| 1 | 단위 테스트 통과 | Vitest (프론트) / xUnit (백엔드) | 전체 케이스 PASS, 신규 기능 테스트 추가 |
...
## Phase 7 — Sprint 7: 통계 화면 목업 🔄 Step 1 진행 중
...
## Phase 8 — Sprint 8: 설정 화면 목업 ⬜ 대기

**유형**: 목업
**목표**: MockupView 설정 탭 UI/UX 목업 확정
**브랜치**: `sprint8` (시작 시 생성)
**DB 스키마 변경**: 없음
**선행 조건**: Sprint 7 Step 2 승인 ✅

| ID | 태스크 | 상태 | 설명 |
|----|--------|------|------|
| T40 | 카드 청구 설정 목업 | ⬜ | 카드별 정산일/결제일 설정 UI |
| T41 | 저축 수단 관리 목업 | ⬜ | 기업은행/카카오뱅크/현금 기본값, 추가/삭제 |
| T42 | 반복 거래 관리 목업 | ⬜ | 활성 반복 목록, 원부 수정/삭제 |
| T43 | 카테고리 관리 목업 | ⬜ | 수입/지출/저축 카테고리 탭 구분 UI |
| T44 | 결제수단 관리 목업 | ⬜ | 카드/현금/포인트 타입 구분, 포인트 잔액 표시 |

---

## Phase 9 — Sprint 9: 전체 실서비스 구현 ⬜ 대기

**유형**: 구현
**목표**: 전체 목업 확정 후 DB + 백엔드 + 프론트 일괄 구현
**브랜치**: `sprint9` (시작 시 생성)
**선행 조건**: Sprint 8 Step 2 승인 (설정 목업 최종 확정) ✅

### 카드 결제 현황 (Sprint 6 Step 3 내용)

| ID | 태스크 | 상태 | 설명 |
|----|--------|------|------|
| T34 | DB 모델 및 마이그레이션 | ⬜ | PaymentMethod에 BillingCutoffDay, PaymentDueDay 추가 |
| T35 | 카드 청구 설정 API | ⬜ | PUT /api/payment-methods/{id} 확장 |
| T36 | 카드 결제 현황 백엔드 API | ⬜ | GET /api/card-billing/summary (2슬롯 계산) + 드릴다운 |
| T37 | 프론트엔드 카드 결제 연동 | ⬜ | UpcomingWidget → 실제 API 교체, SettingsView 카드 설정 연동 |

### 저축 수단

| ID | 태스크 | 상태 | 설명 |
|----|--------|------|------|
| T45 | DB: SavingsMethods 테이블 | ⬜ | 기본 시드 (기업은행, 카카오뱅크, 현금), Transactions FK 추가 |
| T46 | 저축 수단 CRUD API | ⬜ | GET/POST/PUT/DELETE /api/savings-methods |
| T47 | 프론트엔드 저축 수단 연동 | ⬜ | 거래 입력 폼 저축 수단 드롭다운 → 실제 API |

### 통계/설정 UI 실서비스 연동

| ID | 태스크 | 상태 | 설명 |
|----|--------|------|------|
| T48 | 통계 화면 디자인 갱신 | ⬜ | 목업 기반 StatsView 개선 (다크모드, 저축 포함) |
| T49 | 설정 화면 디자인 갱신 | ⬜ | 목업 기반 SettingsView 개선 (저축 수단 관리, 카드 설정 UI) |
...
| Sprint 7 | 목업 | 🔄 진행 중 | 통계 탭 목업 (T38: 도넛 차트/전월 비교, T39: 6개월 막대 추이) — Step 2 승인 대기 | 2026-03-16 |
| Sprint 8 | 목업 | ⬜ 대기 | 설정 화면 전체 목업 (Sprint 7 완료 후 시작) | — |
